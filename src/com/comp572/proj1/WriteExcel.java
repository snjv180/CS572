...
import com.comp572.proj1b.PopulationStatistics;
...
public class WriteExcel {
	...
	Map<String, PopulationStatistics> data = new TreeMap<String, PopulationStatistics>();
	PopulationStatistics popstats = new PopulationStatistics();
	popstats.setGenerationNumer(1);
	popstats.setBestIndividual4Gen("[1 2 3]");
	popstats.setBestIndividualFitness4Gen(0.00);
	popstats.setAvgFitness4Gen(1.23);
	popstats.setTimeToEvolve4Gen(1.23);
	data.put("1", popstats);
	wxl.createSheet(Spherical.FUNCTION_NAME + " Test results", data);
	wxl.closeExcel(Spherical.FUNCTION_NAME + " Test results");
    }
	...
	sheet.setAutoFilter(CellRangeAddress.valueOf("A1:E"+(data.size()+1)));
	...
	try {
	    ...
	    FileOutputStream out = new FileOutputStream(new File("tests"+File.separator+fileName
		    + ".xlsx"));
	    workbook.write(out);
	    out.close();
	    System.out
